...
export function BudgetProgress({
  ...
  const { percent, remaining } = budgetProgress(spent, limit);
  return (
    <section>
      {label ? <p className="text-[15px] text-muted">{label}</p> : null}
      ...
      <p className="mt-2 text-[14px] text-muted">{formatINR(remaining)} remaining</p>
    </section>
  );
}
